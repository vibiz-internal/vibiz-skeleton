...
- **PostHog is pre-installed** (`posthog-js` + `posthog-node` deps + `lib/posthog.tsx` Analytics provider mounted in `app/layout.tsx` + `lib/posthog-server.ts` server client + `instrumentation.ts > onRequestError` for backend errors). Pageviews, pageleaves, click autocapture, **session replay**, frontend `$exception` (window.onerror + unhandledrejection), and backend `$exception` (server actions, route handlers, RSC, middleware) are tracked automatically once the platform injects the env vars.
- **⚠️ DO NOT modify `lib/posthog.tsx`, `lib/posthog-server.ts`, or `instrumentation.ts > onRequestError`. DO NOT add a parallel `lib/posthog-events.ts`, `lib/posthog-capture.ts`, or any file that POSTs to PostHog's `/capture/` endpoint via `fetch`.** Re-implementing PostHog with raw `fetch` looks "simpler" but silently kills session replay (the recorder JS only loads via `posthog-js`), autocapture, rage-click detection, identification cookies, AND the Vibiz auto-bug-task pipeline (Vibiz consumes `$exception` events to create CTO repair tasks per unique fingerprint — bypass the SDK and the pipeline goes silent). The CI guard fails the build if you do. For custom events, just `import posthog from "posthog-js"` and call `posthog.capture()` from anywhere.
